...
def move_candy():
    # loop through list of candh and change y position
...
def update_score_level():
    # use of global since varaibles are changed
...
# Function checks if not on edge updates x coordinates based on right/left
def move_character():
    ...
    if move_direction == "Left" and canvas.coords(mychar)[0] > 0:
       canvas.move(mychar, -10, 0)
    if move_direction == "Up" and canvas.coords(mychar)[0] < 400:
       canvas.move(mychar, 0, -10)
    if move_direction == "Down" and canvas.coords(mychar)[0] > 0:
       canvas.move(mychar, 0, 5)
       
    window.after(16, move_character) # Move character at 60 grames per second

# bind the keys to the character
canvas.bind_all('<KeyPress>', check_input) # bind key press
canvas.bind_all('<KeyRelease>', end_input) # bind all eksy to circle

# Start game loop by scheduling all the functions 
window.after(1000, end_title) # destory title and instructions
...
window.mainloop() # last line in the GUI main event loop